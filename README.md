...
needed. Send `/stop` to unsubscribe.
...
- The first hourly observation above the threshold marks the cover as changed,
  silently cancels any remaining reminders, and completes monitoring for that Reel.
...
threshold can cause an unnecessary or missed reminder. Messages therefore say that
no change was **detected**, not that Instagram confirmed the cover is unchanged.
The worker does not publish posts or change covers.
...
- `status` shows post, reminder, subscriber, and per-recipient delivery counts,
  update-poll status, and the last cycle's errors. Health is unhealthy
...
- An inaccessible/deleted Reel does not produce a reminder; its repeated check
  failures appear in logs and health.
...
- Stop the worker before backing up the entire data volume. Restore that volume
  to preserve activation and reminder history. Do not remove the
...
`run --once` respects the persisted hourly schedule.
